...
d3.csv("../data/birth_data.csv").then(data => {
    ...
    data.forEach(d => {
        ...
        // Creating a Map with key: year and value: another Map() i.e., yearData
        if (!stateDataByYear.has(year)) {
            stateDataByYear.set(year, new Map());
        }
        
        // yearData is a Map()  with key: stateFIPS and value: aggregated births
    ...
    // Calculate and store normalizedBirths
    stateDataByYear.forEach(yearData => {
        yearData.forEach((births, stateFIPS) => {
            let normalizedBirths = (births - globalMinBirths) / (globalMaxBirths - globalMinBirths);
            yearData.set(stateFIPS, { births, normalizedBirths }); // Store both values
        });
    });

    console.log("Normalized Birth Data by Year:", stateDataByYear);
            ...
            function updateMap(selectedYear) {
                ...
                svg.selectAll(".state")
                    ...
                    .attr("fill", d => {
                        ...
                        let normalizedBirths = yearData.get(stateFIPS)?.normalizedBirths ?? 0; // Default to 0 if missing
                        console.log(normalizedBirths);
                        return colorScale(normalizedBirths);
                    });
